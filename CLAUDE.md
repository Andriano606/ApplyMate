...
- `.ai/docs/scrapers.md` — Scraper base interface, vacancy struct shape, `ApplyMate::Client::AsyncHttp` / `ImpersonateHttp` API, CSRF session init pattern, graceful termination, adding a new scraper. **Read before adding or modifying scrapers.**
...
| Scrapers, Client::AsyncHttp/ImpersonateHttp/Browser, apply handlers, vacancy sync, proxies, fibers, AI prompts | `pipeline-engineer` | opus / high |
