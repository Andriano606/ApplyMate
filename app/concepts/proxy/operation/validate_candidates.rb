...
class Proxy::Operation::ValidateCandidates < ApplyMate::Operation::Base
  ...
  def perform!(candidates:, **)
    ...
    log_structured(event: 'proxy_validation_started', candidates: filtered.size)
    ...
    if valid.empty?
      ...
      log_structured(event: 'proxy_validation_no_results', candidates: filtered.size, level: :warn)
    else
      rate = (valid.size * 100.0 / [ filtered.size, 1 ].max).round(2)
      log_structured(event: 'proxy_validation_completed', valid: valid.size, total: filtered.size, rate: rate)
    end
