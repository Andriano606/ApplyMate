...
class Apply::Ai::ResponseSchema::FillForm < ApplyMate::Ai::ResponseSchema::Base
  def self.format_instructions
    <<~INSTRUCTIONS
      ...
      Поверни результат виключно у форматі JSON об'єкта, де ключ — це id поля (значення у квадратних дужках на початку рядка), а значення — це текст для введення. Не додавай жодних зайвих пояснень чи Markdown оформлення (крім самого блоку коду).
      Включи кожне перелічене поле.
    INSTRUCTIONS
  end
