...
class ApplyMate::Operation::Base
  ...
  # autohide nil gives default behaviour
  def notice(text, level: :notice, autohide: nil)
    @result[:notice] = {
      text:,
      level:,
      autohide:
  ...
  def authorize!(record, query, policy: Pundit.policy!(@attrs[:current_user], record), fail_message: nil)
    raise 'Please provide a model' if %i[update? destroy? create? edit? new?].include?(query) &&
                                      !(record.is_a?(ApplicationRecord) ||
                                        record.is_a?(ActiveModel::Model) ||
